...
The first run will be starting from 2019-01-01 (from 2019 to 2020).
```
...
When we set it to monthly, the DAG will run once per "logical" month, but we can run all available months at once.
```
...
```
12 DAG runs are green (12 months of 2019).
```
...
```
Once. We need to download only 1 file.
```
